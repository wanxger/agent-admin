...
pnpm test       # Run unit tests (Vitest)
pnpm test --run # Run unit tests once (without watch mode)
TEST_INTEGRATION=true pnpm test # Run integration tests (requires opencode acp)
```
...
#### Integration Tests
Integration tests require the `opencode acp` command to be available. Run with:
```bash
cd libs/prompt && TEST_INTEGRATION=true pnpm test
```
